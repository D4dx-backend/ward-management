        ...
        if (field.subQuestions && field.subQuestions.length > 0) {
          field.subQuestions.forEach((subQuestion, subIndex) => {
            ...
            if (subValue !== undefined) {
              apiResponses[`${field.label} - ${subQuestion.label}`] = subValue;
            }
          });
        }
      });
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600">
              {report.formTemplate?.title || 'Coordinator Report'} - Week {report.weekNumber}, {report.year}
            </p>
          </div>
        ...
        <Card>
          <div className="p-6 border-b border-gray-200">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div>
                <h3 className="text-sm font-medium text-gray-500">District Information</h3>
                <div className="mt-2 space-y-1">
                  <p className="text-sm text-gray-900">District: {report.district || session.user.district}</p>
                </div>
              </div>
              <div>
            ...
            {!showPreview ? (
              <>
                <div className="flex items-center justify-between mb-4 p-4 bg-gray-50 rounded-lg">
                  ...
                  <Link href={`/coordinator/reports/view/${report._id}`}>
