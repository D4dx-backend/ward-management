...
export default function WardVisitsAnalysis() {
  ...
  useEffect(() => {
    ...
    } else if (status === 'authenticated' && !['stateAdmin', 'coordinator'].includes(session.user.role)) {
      router.push('/');
  ...
  const fetchData = async () => {
    try {
      ...
      let visitsEndpoint, wardsEndpoint, coordinatorsEndpoint, statsEndpoint;
      
      if (session.user.role === 'stateAdmin') {
        visitsEndpoint = '/api/admin/ward-visits';
        wardsEndpoint = '/api/wards/';
        coordinatorsEndpoint = '/api/users/?role=coordinator';
        statsEndpoint = '/api/admin/ward-visits/statistics';
      } else if (session.user.role === 'coordinator') {
        visitsEndpoint = '/api/ward-visits/';
        wardsEndpoint = '/api/wards/';
        coordinatorsEndpoint = '/api/users/?role=coordinator';
        statsEndpoint = '/api/admin/ward-visits/statistics';
      }
      ...
      const requests = [
        ...
        axios.get(wardsEndpoint),
        axios.get(coordinatorsEndpoint)
      ];
      
      // Only add statistics for admin
      if (session.user.role === 'stateAdmin') {
        requests.push(axios.get(statsEndpoint));
      }
      
      const responses = await Promise.all(requests);
      
      setVisits(responses[0].data || []);
      setWards(responses[1].data || []);
      setCoordinators(responses[2].data || []);
      
      if (responses[3]) {
        setStatistics(responses[3].data || {});
      } else {
        // Calculate basic statistics for coordinator
        const visitsData = responses[0].data || [];
        setStatistics({
          totalVisits: visitsData.length,
          visitsThisMonth: visitsData.filter(v => {
            const visitDate = new Date(v.visitDate);
            const now = new Date();
            return visitDate.getMonth() === now.getMonth() && visitDate.getFullYear() === now.getFullYear();
          }).length,
          followUpPending: visitsData.filter(v => v.followUpRequired && !v.followUpCompleted).length,
          followUpOverdue: visitsData.filter(v => 
            v.followUpRequired && 
            !v.followUpCompleted && 
            v.followUpDate && 
      ...
      setError('Failed to fetch ward visits data. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600">
              {session.user.role === 'stateAdmin' 
                ? 'Monitor and analyze all coordinator ward visits across the state'
                : 'Monitor and analyze ward visits under your coordination'
              }
            </p>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  {wards.map((ward) => (
                    <option key={ward._id} value={ward._id}>{ward.name} (Ward #{ward.wardNumber})</option>
                  ))}
                </select>
              </div>
          ...
          <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
            <div className="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
              ...
              <div className="flex justify-end space-x-3 pt-6 border-t mt-6">
                ...
                {session.user.role === 'coordinator' && selectedVisit.coordinator?._id === session.user.id && (
                  <Link href={`/coordinator/ward-visits?edit=${selectedVisit._id}`}>
                    <Button>
                      Edit Visit
                    </Button>
                  </Link>
