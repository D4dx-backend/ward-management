...
1. The existing survey document contained House Visits from multiple wards mixed together
...
## Root Cause
The survey document in the database likely contains House Visits from other wards that got mixed in somehow. The automatic sync logic was only adding new clusters but not removing clusters that don't belong to the current ward.
...
**Changes:**
- Added logic to filter out House Visits that don't belong to the current ward
- Remove invalid House Visits before processing new ones
- Ensure only clusters from the current ward are included in the response

### 2. **Automatic Cleanup Logic**
```javascript
// First, filter out any House Visits that don't belong to this ward
const currentClusterIds = new Set(clusters.map(c => c._id.toString()));

// Remove House Visits that don't belong to this ward
...
if (validClusterVisits.length !== survey.clusterVisits.length) {
  console.log(`Removed ${survey.clusterVisits.length - validClusterVisits.length} invalid House Visits`);
...
Removed 6 invalid House Visits
```
...
```
Found 6 clusters for ward 68846eed430c7b20683e76d1
Removed 3 invalid House Visits
